...
export default class WareHouseScan extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      CodeToScanList: props.route.params.CodeToScanList, // For out House
      ScanStatus: props.route.params.ScanStatus,
      CartItemNumber: 0,
  ...
  CameraFlash = () =>{
   let array = this.state.camera;
   
   if(array['flashMode'] == RNCamera.Constants.FlashMode.off){ // Turn on
    array['flashMode'] = RNCamera.Constants.FlashMode.torch;
    this.setState({camera: array});
   }else{
   
      array['flashMode'] = RNCamera.Constants.FlashMode.off; // Turn light off
      this.setState({camera: array});
   }
  }
      ...
      <View style={styles.container}>
        ...
        <RNCamera
         ...
         //  onGoogleVisionBarcodesDetected={this.barcodeRecognized.bind(this)}
            ...
            autoFocus={this.state.camera.focus}
            mirrorImage={false}
            onFocusChanged={() => {}}
            onZoomChanged={() => {}}
        ...
        <View style={[styles.overlay, styles.topOverlay]}>
          <Icon
            ...
            name="qrcode-scan"
            size={24}/> 
          {this.state.ScanStatus !== 'Return' && 
          <Text style={styles.scanScreenMessage}>Mời quét mã QR sản phẩm</Text>
          }
          {this.state.ScanStatus === 'Return' && 
            <Text style={styles.scanScreenMessage}>Mời quét mã QR: {this.state.CodeToScanList.map(item => item.itemSKUCode.toUpperCase())}</Text>
          }
          <View>
        ...
        <View style={[styles.overlay, styles.bottomOverlay]}>
          <View style={styles.NaviBlock}>
            <TouchableOpacity style={styles.cameraAdjBtn} onPress={this.CameraFlash}>
              <Icon
                style={styles.InputIcon}
                name='flashlight'
                size={25}/>
              {this.state.camera.flashMode == RNCamera.Constants.FlashMode.torch &&
                <Text style={styles.CameraBtn}>Tắt đèn</Text>
              }  
              {this.state.camera.flashMode == RNCamera.Constants.FlashMode.off &&
                <Text style={styles.CameraBtn}>Bật đèn</Text>
              }  
            </TouchableOpacity>
